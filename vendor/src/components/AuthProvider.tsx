  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        console.log('🔐 Local storage check - Access token:', !!localToken, 'Refresh token:', !!refreshTokenValue);
        
        // If we have a token in localStorage but not in the store, set it in the store
        if (localToken && !accessToken) {
          console.log('🔄 Setting access token from localStorage to auth store');
          setTokenAndUser(localToken, null);
        }
        
        // If we have a token in the store but not in localStorage, save it there
        if (accessToken && !localToken) {
          console.log('🔄 Saving access token from store to localStorage');
          localStorage.setItem('accessToken', accessToken);
        }
        
        // Initialize auth (this will restore from storage and validate)
        const restored = await initializeAuth();
        console.log('🔐 Auth Provider: Auth initialized, authenticated:', restored);
        
        // If not restored but we have a refresh token, try explicit refresh
        if (!restored && refreshTokenValue) {
          console.log('🔄 Auth not restored but refresh token exists, trying explicit refresh...');
